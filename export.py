...
if __name__ == '__main__':
    ...
    for _ in range(2):
        y_orig = model(img)  # dry runs
    output_names = None
    if opt.export_nms:
        nms = models.common.NMS(conf=0.001, kpt_label=True)

        nms_export = models.common.NMS_Export(conf=0.001, kpt_label=True)

        y = nms(y_orig)
        print("NMS y_export: ", y)

        y_export = nms_export(y_orig)
        print("NMS y_export: ", y_export)
